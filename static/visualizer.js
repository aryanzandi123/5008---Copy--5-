...
let pathwayNodeRadius = 45;          // Size for pathway nodes
let pathwayRingRadius = 350;         // Distance from center for pathway nodes
let expandedPathways = new Set();    // Set of expanded pathway IDs
...
function buildInitialGraph(){
  ...
  // Check if pathway mode should be enabled
  pathwayMode = pathways.length > 0;

  if (pathwayMode) {
    // PATHWAY MODE: Create pathway nodes instead of interactor nodes
    console.log(`🛤️ Pathway mode enabled: ${pathways.length} pathways`);

    pathways.forEach((pw, idx) => {
      const pathwayId = pw.id || `pathway_${pw.name.replace(/\s+/g, '_')}`;
      const angle = (idx / pathways.length) * 2 * Math.PI - Math.PI / 2;
...
function createSimulation(){
  ...
  node.each(function(d){
    ...
    if (d.type === 'main') {
      ...
      // Pathway label (truncated if long)
      const truncLabel = d.label.length > 14 ? d.label.substring(0, 12) + '...' : d.label;
      group.append('text')
      ...
      // Interactor count badge
      const count = (d.interactorIds || []).length;
      if (count > 0) {
        group.append('circle')
          .attr('class', 'pathway-badge')
          .attr('cx', pathwayNodeRadius * 0.7)
          .attr('cy', -pathwayNodeRadius * 0.7)
          .attr('r', 12)
          .style('fill', '#ef4444');

        group.append('text')
          .attr('class', 'pathway-badge-text')
          .attr('x', pathwayNodeRadius * 0.7)
          .attr('y', -pathwayNodeRadius * 0.7)
          .attr('text-anchor', 'middle')
          .attr('dy', 4)
          .style('fill', 'white')
          .style('font-size', '10px')
      ...
      // Interactor node (standard or under pathway)
      group.append('circle')
    ...
    if (isIndirectInteraction) {
      ...
      functionsHTML += `<div style="margin:16px 0;">
        ${functions.map(f => {
          ...
          return renderExpandableFunction(f, SNAP.main, L.primary, interactionArrow, direction);
        }).join('')}
      </div>`;
  ...
  function renderInteractionSection(link, sectionType) {
    ...
    if (functions.length > 0) {
      functionsHTML = functions.map(fn => {
        // Pass SNAP.main and interactorName to ensure correct direction resolution
        return renderExpandableFunction(fn, SNAP.main, interactorName, arrow, direction);
      }).join('');
    } else {
