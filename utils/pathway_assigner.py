...
def build_pathway_generation_prompt(
    ...
    """
    Build prompt for Stage 1: Generate pathway names for interactions.
    """
    # Extract relevant info from interactors
    interaction_summaries = []
    for ix in interactors:
        primary = ix.get("primary", "?")
        arrow = ix.get("arrow", "binds")
        functions = ix.get("functions", [])
        function_names = [f.get("function", "") for f in functions[:5]]  # Top 5 functions
    ...
    prompt = f"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   PATHWAY ASSIGNMENT – STAGE 1: GENERATE BIOLOGICAL PATHWAY CATEGORIES       ║
╚══════════════════════════════════════════════════════════════════════════════╝

ROLE: You are an expert molecular biologist categorizing protein-protein interactions
      into biological pathways and processes.

CONTEXT:
  Main/Query Protein: {main_protein}
  Number of interactions: {len(interactors)}

OBJECTIVE:
  Analyze ALL interactions and assign each to ONE OR MORE biological pathways.
  Use standard biological terminology (KEGG, Reactome, GO-style names preferred).

RULES:
  1. EVERY interaction MUST be assigned to at least ONE pathway (no uncategorized).
  2. If an interaction doesn't fit existing pathways, CREATE a new descriptive pathway.
  ...
  5. For novel/unclear interactions, create descriptive pathway names based on
     the molecular mechanism or biological context.

GOOD PATHWAY NAMES:
  - "Autophagy" (specific process)
  - "mTOR Signaling" (specific pathway)
  - "ER-Associated Degradation" (specific mechanism)
  - "Oxidative Stress Response" (specific response)
  - "Polyglutamine Protein Quality Control" (specific context)

BAD PATHWAY NAMES (avoid these):
...
{{
  "pathways": {{
    "PATHWAY_NAME": {{
      "description": "Brief (≤100 char) description of the pathway",
      "interactors": ["SYMBOL1", "SYMBOL2", ...],
      "confidence": 0.8
    }},
    "ANOTHER_PATHWAY": {{
      "description": "...",
...
NOTE: An interactor CAN appear in multiple pathways if biologically relevant.

────────────────────────────────────────────────────────────────────────────────
INTERACTION DATA (READ CAREFULLY)
────────────────────────────────────────────────────────────────────────────────
{context_json}
"""
    return prompt
...
def generate_pathway_names(
    ...
    """
    Stage 1: Use Gemini to generate pathway assignments for interactions.
    ...
    Returns:
        Tuple of:
        - pathways: Dict of pathway_name -> {description, interactors, confidence}
        - updated_interactors: Interactors with 'pathways' field added
    """
    if not interactors:
        return {}, []
    ...
    if verbose:
        print(f"[PathwayAssigner] Stage 1: Generating pathways for {len(interactors)} interactions...")

    result = _call_gemini_json(prompt, api_key)
    pathways = result.get("pathways", {})

    if verbose:
        print(f"[PathwayAssigner] Generated {len(pathways)} pathways")

    # Build reverse mapping: interactor -> list of pathways
    interactor_pathways: Dict[str, List[Dict]] = {}
    for pw_name, pw_data in pathways.items():
        for interactor in pw_data.get("interactors", []):
            if interactor not in interactor_pathways:
                interactor_pathways[interactor] = []
            interactor_pathways[interactor].append({
                "name": pw_name,
                "confidence": pw_data.get("confidence", 0.8),
            })

    # Update interactors with pathway assignments
    updated_interactors = []
    for ix in interactors:
        ix_copy = dict(ix)
        primary = ix.get("primary", "")
        ix_copy["pathways"] = interactor_pathways.get(primary, [])

        # If no pathway assigned (shouldn't happen), create one from function names
        if not ix_copy["pathways"]:
            functions = ix.get("functions", [])
            if functions:
                func_name = functions[0].get("function", "Unknown Process")
                ix_copy["pathways"] = [{"name": func_name, "confidence": 0.5}]
            else:
                ix_copy["pathways"] = [{"name": "Protein Interaction", "confidence": 0.3}]
...
def apply_pathway_mappings(
    ...
    """
    Stage 3: Apply consolidated pathway mappings to interactors.

    Updates each interactor's 'pathways' array with:
    - canonical_name: Final pathway name
    - ontology_id: Standard ontology ID (if mapped)
    - ontology_source: GO/KEGG/Reactome (if mapped)
    - confidence: Assignment confidence

    Args:
        interactors: Interactors with 'pathways' field from Stage 1
        consolidated: Consolidated mappings from Stage 2

    Returns:
        Updated interactors with enriched pathway metadata
    """
    updated = []
    for ix in interactors:
        ix_copy = dict(ix)
